...
{
    screen = newscreen;
    cout<<screen.w<<"changed screen"<<endl;
}
...
void Renderer::renderSprite(Sprite* sprite,float x,float y) //simplest
{
    ...
    SDL_Rect frame = sprite->getCurrentFrame();
    SDL_Texture* src_img = sprite->getTexture();
    //cout <<screen.x<<"renderer screen x"<<endl;
    //get destination 
    SDL_Rect drect = getDestinationRect(x,y,frame.w,frame.h,1);
    if(!isInsideScreen(drect))
    {
        cout<<"is outisde screen"<<endl;
        return ;
    }
...
{
   // cout << screen.w<<"can this print "<<endl;
   cout <<" x = "<< r.x<<" y = "<<r.y<<endl;
   cout <<" screen x = "<<screen.x<<" screen y =  "<<screen.y<<endl; //this is fishy
...
SDL_Texture* Renderer::createTexturefromSurface(SDL_Surface* surface)
{
    ...
    if(t == nullptr)
    {
        cerr << "Couldnt create texture "<<endl;
    }
    return t;
}